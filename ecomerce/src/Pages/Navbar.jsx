...
const Navbar = () => {
 const navigate= useNavigate();
  ...
  return (
    ...
    <Link to={'/'}>  <h1 className="text-4xl font-bold text-white font-serif">
      ...
      <div className="flex border p-1 space-x-1 bg-white rounded-full items-center  outline-green-900 active:outline-4 transition-all my-5 md:my-0">
        <input type="text" value={keyword} onChange={(e)=>{setkeyword(e.target.value)}} className="bg-white outline-0 ps-3 rounded-2xl italic font-semibold overflow-hidden flex-1"  placeholder="Enter the Product"/>
        <button>{<FaSearch className="text-red-600 hover:cursor-pointer text-xl hover:text-green-700" />}</button>
      </div>
      </form>
